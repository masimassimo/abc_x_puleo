...
class abc_contacts_sales_commissions(models.Model):
    ...
    name = fields.Char(string="Nome", help = "Il nome dato alla regola di provvigione.", store=True, required=True, tracking=True, index = True, copy = False, readonly = True, default=lambda self: _('New'))
    ...
    #Campo contatto_preferito che permette di discriminare una regola di provvigione standard da un'altra così se quell'agente viene assegnato al contatto_preferito prenderà la provvigione massima.
    contatto_preferito = fields.Many2one("res.partner", string = "Contatto preferito", help = "Campo che permette di discriminare una regola di provvigione standard da un'altra specifica per contatto. Se questo ageste viene assegnato al contatto_preferito prenderà la provvigione massima.", tracking = True)
